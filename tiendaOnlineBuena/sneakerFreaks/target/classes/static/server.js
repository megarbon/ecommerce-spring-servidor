        ...
        .then(data => {
            if (data.length > 0) {
                ...
                keys.forEach(key => {
                    ...
                    th.classList.add('px-4', 'py-2', 'text-left', 'border-b', 'border-gray-400', 'bg-gray-100', 'text-sm', 'font-semibold', 'text-gray-700');
                    th.textContent = key;
                ...
                data.forEach(item => {
                    ...
                    keys.forEach(key => {
                        ...
                        td.classList.add('px-4', 'py-2', 'border-b', 'border-gray-400', 'text-sm', 'text-gray-700');
                ...
                const resultDiv = document.getElementById('consultarProductosResult');
                resultDiv.innerHTML = '';
                resultDiv.appendChild(table);
            } else {
                document.getElementById('consultarProductosResult').innerHTML = 'No hay datos disponibles.';
            }
        })
    ...
    fetch(`http://localhost:8080/products/${idConsulta}`)
        ...
        .then(data => {
            if (data) {
                ...
                document.getElementById('consultarProductoPorIdResult').innerHTML = mensaje;
            } else {
                document.getElementById('consultarProductoPorIdResult').innerHTML = 'No se encontró el producto.';
            }
        })
        .catch(error => console.error('Error:', error));
}
...
function eliminarProducto() {
    ...
    fetch(`http://localhost:8080/products/delete/${idEliminar}`, {
        ...
        .then(response => {
            if (response.ok) {
                document.getElementById('eliminarProductoResult').innerHTML = 'Producto eliminado correctamente.';
            } else {
                document.getElementById('eliminarProductoResult').innerHTML = 'Error al eliminar el producto.';
            }
        })
        .catch(error => console.error('Error:', error));
}
    ...
    fetch(`http://localhost:8080/products/update/${idModificar}`, {
        ...
        .then(data => {
            if (data.id) {
                ...
                // Mensaje de error
                document.getElementById('modificarProductoResult').innerHTML = 'Error al modificar el producto. Asegúrate de proporcionar un ID válido.';
            }
        })
        .catch(error => console.error('Error:', error));
}
